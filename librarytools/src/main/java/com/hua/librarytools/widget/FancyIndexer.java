package com.hua.librarytools.widget;
...
//				mScroller.startScroll(0, 0, 255, 0, 1000);
    ...
    private void commonData(int top, int bottom) {
        paddingTop = DensityUtil.dip2px(mContext,top);
        paddingBottom = DensityUtil.dip2px(mContext,bottom);
    ...
    public void setParams(String[] constChar,int top,int bottom){
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < mConLength; i++) {
            ...
            //Log.v("hjz", "fontSize=" + DisplayUtils.px2sp(mContext, fontSize));
            ...
            if (isUp) {
                if(mChooseIndex > -1){
                    //Log.v("hjz", "选中的字母下标mChooseIndex=" + mChooseIndex);
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (this.getWidth() > mWidthOffset) {
                    if (event.getX() < this.getWidth() - mWidthOffset-DensityUtil.dip2px(mContext,5))
                        return false;
                }
